    ...
    while let Some(event) = rx.recv().await {
        match event {
            StreamEvent::TextDelta(delta) => {
                println!("[TextDelta] {:?}", delta);
                print!("{}", delta);
                full_text.push_str(&delta);
            }
            StreamEvent::ReasoningDelta(delta) => {
                println!("[ReasoningDelta] {:?}", delta);
...
async fn test_dashscope_simple_input() {
    ...
    while let Some(event) = rx.recv().await {
        match event {
            ...
            StreamEvent::Done(resp) => {
                println!("\n[完成] 模型：{}, 用量：{:?}", resp.model, resp.usage);
                println!("[Debug] full_text={:?}", full_text);
                println!("[Debug] full_reasoning={:?}", full_reasoning);
            }
            StreamEvent::Error(e) => {
                panic!("Stream error: {}", e);
            }
