...
- `[ ]` 016 tool pool changes tracked across startup and runtime activation
- `[ ]` 017 deferred tool activation telemetry
...
- `[ ]` 042 resume warmup progress staged into startup profile
- `[ ]` 043 resume cache hit/miss counters by transcript metadata cache
- `[ ]` 044 resume path split between metadata-only and full transcript restore
- `[ ]` 045 restored message decode metrics
- `[ ]` 046 db open / load timing surfaced in startup profile
- `[ ]` 047 transcript cache invalidation diagnostics
- `[ ]` 048 session restore fallback reason reporting
- `[ ]` 049 transcript benchmark summary exposed in `/doctor`
...
- `[ ]` 090 tool execution artifact cross-links in status
