...
- 2026-05-19: Extended `discover_skills` with optional ranked `query` results, deterministic no-query ordering, trigger-example details, and metadata scores/reasons. Verified with `cargo test -q -p yode-core skills`, `cargo test -q -p yode-tools skill`, `cargo test -q -p yode-tui skills`, `cargo check -q`, and `git diff --check`. Remaining risk: skill invocation persistence across compaction is still pending.
- 2026-05-19: Added session/subagent skill invocation records for successful `skill` get/run calls, including team/member scope propagation through subagent contexts. Verified with `cargo test -q -p yode-tools skill`, `cargo test -q -p yode-core subagent_context_carries_invocation_scope`, `cargo test -q -p yode-core skills`, `cargo test -q -p yode-tui skills`, `cargo check -q`, and `git diff --check`. Remaining risk: invoked skill excerpts are not yet restored through compaction, and `/skills active` still does not explain invocation reasons.
- 2026-05-19: Restored recent invoked skills through compact by sharing invocation records with the engine and including budgeted skill excerpts in the post-compact skills block. Verified with `cargo test -q -p yode-core compaction`, `cargo test -q -p yode-tools skill`, `cargo test -q -p yode-core skills`, `cargo test -q -p yode-tui skills`, `cargo check -q`, and `git diff --check`. Remaining risk: `/skills active` still does not explain invocation reasons, and stale referenced-file diagnostics remain pending.
- 2026-05-19: Added `/skills active` match explanations by returning matched path gates and concrete paths from skill activation. Verified with `cargo test -q -p yode-core skills`, `cargo test -q -p yode-tui skills`, `cargo test -q -p yode-tools skill`, `cargo check -q`, and `git diff --check`. Remaining risk: stale referenced-file diagnostics remain pending.
- 2026-05-19: Added stale skill diagnostics for missing explicitly referenced skill files, surfaced them in `/skills list` and `/diagnostics`. Verified with `cargo test -q -p yode-core skills`, `cargo test -q -p yode-tui skills`, `cargo test -q -p yode-tools skill`, `cargo check -q`, and `git diff --check`. Remaining risk: richer stale diagnostics for invalid skill frontmatter can be added later.
...
- 2026-05-19: Extended remote replay to reconstruct transport status, reconnect count, and queue item status summaries from JSONL events. Verified with `cargo test -q -p yode-tools remote_runtime`, `cargo test -q -p yode-tui remote_control`, `bash scripts/parity-replay-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: replay does not yet materialize recovered state artifacts, and remote storage backend abstraction is still pending.
- 2026-05-19: Introduced a narrow remote storage backend boundary for tool-side transport event artifacts and JSONL appends, preserving local filesystem behavior. Verified with `cargo test -q -p yode-tools remote_runtime`, `cargo test -q -p yode-tui remote_control`, `bash scripts/parity-replay-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: other remote artifacts and the TUI mirror still write directly to local filesystem.
- 2026-05-19: Mirrored the remote storage boundary in TUI transport event recording so markdown event artifacts and JSONL appends share a backend seam. Verified with `cargo test -q -p yode-tools remote_runtime`, `cargo test -q -p yode-tui remote_control`, `bash scripts/parity-replay-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: non-event remote artifacts still write directly to local filesystem.
- 2026-05-19: Routed TUI remote control plan artifacts through the local remote storage seam while preserving artifact names and payloads. Verified with `cargo test -q -p yode-tools remote_runtime`, `cargo test -q -p yode-tui remote_control`, `bash scripts/parity-replay-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: transport/live-session/result artifacts still write directly to local filesystem.
...
- 2026-05-19: Promoted `/keybindings` as an alias for `/keys`, expanded the output with built-in keymap source plus user/project/local config path availability, and linked it from `/help` and `/status`. Verified with `cargo test -q -p yode-tui keys`, `cargo test -q -p yode-tui commands`, `cargo test -q -p yode-core db`, `cargo check -q`, `cargo fmt -p yode -p yode-core -p yode-llm -p yode-tools -p yode-tui -p yode-mcp -p yode-agent -- --check`, and `git diff --check`. Remaining risk: keybindings are still built-in defaults; this does not add user-editable keymap loading.
...
- 2026-05-20: Made `/diagnostics` top-issue rows use the current terminal width instead of a fixed 96-column cap and added focused narrow/normal/wide width coverage for CJK plus inspect hints. Verified with `cargo test -q -p yode-tui diagnostics`. Remaining risk: lower legacy sections still rely on scrollback wrapping rather than per-field responsive truncation.
- 2026-05-20: Made `/diagnostics` Observability rows width-aware so long hook/team/remote/startup/MCP/plugin/skill artifact summaries truncate by visible terminal width instead of overflowing narrow screens. Added focused narrow-width coverage for long remote artifact paths. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: Context, Memory, Recovery, Tools, Hooks, and Timeline legacy sections still rely on scrollback wrapping rather than per-field responsive truncation.
- 2026-05-20: Made `/diagnostics` Tools rows width-aware so long progress summaries, error type lists, and latest tool artifact paths truncate by visible terminal width on narrow screens. Added focused narrow-width coverage for long tool artifact paths and error keys. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: Context, Memory, Recovery, Hooks, and Timeline legacy sections still rely on scrollback wrapping rather than per-field responsive truncation.
- 2026-05-20: Made `/diagnostics` Recovery rows width-aware so long recovery signatures, permission explanations, and recent denial commands truncate by visible terminal width on narrow screens. Added focused narrow-width coverage for repeated failure and permission denial text. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: Context, Memory, Hooks, and Timeline legacy sections still rely on scrollback wrapping rather than per-field responsive truncation.
- 2026-05-20: Made `/diagnostics` Context rows width-aware so long query sources, compact breaker reasons, token counts, and media compact summaries truncate by visible terminal width on narrow screens. Added focused narrow-width coverage for long context status text. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: Memory, Hooks, and Timeline legacy sections still rely on scrollback wrapping rather than per-field responsive truncation.
- 2026-05-20: Made `/diagnostics` Memory rows width-aware so long live-memory status and latest memory artifact paths truncate by visible terminal width on narrow screens. Added focused narrow-width coverage for long session memory paths. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: Hooks and Timeline legacy sections still rely on scrollback wrapping rather than per-field responsive truncation.
- 2026-05-20: Made `/diagnostics` Timeline rows width-aware so long hook/task/runtime event summaries truncate by visible terminal width on narrow screens without changing timeline ordering or source data. Added focused narrow-width coverage for long hook failure and timeout timeline entries. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: the Hooks summary rows are still numeric-only and have not been moved to the shared row renderer.
- 2026-05-20: Moved `/diagnostics` Hooks summary rows onto the shared width-aware row renderer, keeping the same numeric fields while making the section consistent with the rest of the overview. Added focused narrow-width coverage for hook count rows. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: the top runtime summary lines are still compact prose summaries and can wrap naturally on very narrow terminals.
- 2026-05-20: Moved `/diagnostics` Runtime/Context/Tool summary rows onto the shared width-aware row renderer so long compact prose summaries truncate by visible terminal width before the detailed sections. Added focused narrow-width coverage for long context summary text. Verified with `cargo test -q -p yode-tui diagnostics`, `bash scripts/parity-visual-ci.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: extremely narrow terminals below the renderer floor still rely on the 32-column minimum.
- 2026-05-21: Made `/context` runtime detail rows width-aware so long prompt-cache, compact, memory, and async-task summaries truncate by visible terminal width instead of overflowing narrow screens. Added focused narrow-width coverage for the runtime detail block. Verified with `cargo test -q -p yode-tui context_cmd`, `cargo check -q`, and `git diff --check`. Remaining risk: the compact context mix bar still uses a fixed 30-character bar and relies on its existing compact layout rather than terminal-specific scaling.
- 2026-05-21: Made the `/context` mix bar and legend scale by terminal width, using compressed role labels on very narrow terminals and a wider bar on roomy terminals while preserving visible nonzero segments. Added focused 32-column coverage for the mix block. Verified with `cargo test -q -p yode-tui context_cmd`, `cargo check -q`, and `git diff --check`. Remaining risk: the mix percentages still use display/runtime estimates rather than provider-specific tokenizer accounting.
...
- 2026-05-20: Added a generated parity contract failure triage template sourced from `docs/optimization/parity-contract-manifest.tsv`, with owner, CI job, uploaded artifact, fixtures/docs, focused reruns, and closeout fields per contract. `scripts/parity-artifact-bundle.sh` now includes the generated template in uploaded parity bundles, while docs CI keeps the checked-in template reproducible. Verified with `bash scripts/parity-contract-triage-template.sh`, `bash scripts/parity-artifact-bundle.sh .yode/parity-artifacts`, `bash scripts/parity-docs-ci.sh`, `bash scripts/parity-ci-local.sh`, `bash scripts/parity-risk-register-validate.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: the template points to bundle artifacts and focused reruns, but it does not yet parse a failed GitHub run log automatically.
...
- 2026-05-20: Added `docs/optimization/304-four-month-release-note-draft.md` via `scripts/release-candidate-note.sh`, summarizing only user-visible release-candidate changes and explicitly avoiding claims about Claude-hosted remote infrastructure, model-native watch behavior, or server-managed policy delivery. Wired the note into docs CI, syntax dry-run coverage, and parity artifact bundles. Verified with `bash scripts/release-candidate-note.sh`, `bash scripts/parity-docs-ci.sh`, `bash scripts/parity-ci-local.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: final public release notes still need tag-specific pruning after the GitHub Actions matrix finishes.
- 2026-05-20: Added `docs/optimization/305-release-benchmark-evidence.md` via `scripts/release-benchmark-evidence.sh`, validating the long-session benchmark snapshot contract and documenting how to compare the current CI-uploaded benchmark against the golden/current artifact before tagging. Wired the evidence doc into docs CI, syntax dry-run coverage, and parity artifact bundles. Verified with `bash scripts/release-benchmark-evidence.sh`, `bash scripts/parity-benchmark-ci.sh`, `bash scripts/parity-docs-ci.sh`, `bash scripts/parity-ci-local.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: the evidence report intentionally avoids volatile local millisecond values; final performance claims still require reviewing the CI artifact from the release run.
- 2026-05-20: Added `docs/optimization/306-release-validation-matrix.md` via `scripts/release-validation-matrix.sh`, validating that CI runs workspace library tests on Linux/macOS/Windows, keeps the release clippy gate, runs parity jobs, and uploads benchmark/parity artifacts. Wired the matrix doc into docs CI, syntax dry-run coverage, and parity artifact bundles. Verified with `bash scripts/release-validation-matrix.sh`, `cargo test --workspace --lib`, `cargo clippy -p yode -p yode-core -p yode-llm -p yode-tools -p yode-tui -p yode-mcp -p yode-agent --no-deps -- -D warnings`, `bash scripts/parity-docs-ci.sh`, `bash scripts/parity-ci-local.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: local validation cannot replace the remote GitHub Actions matrix result before tagging.
- 2026-05-20: Added a README release-candidate validation entry that points maintainers to `bash scripts/release-checklist.sh`, the release note draft, benchmark evidence, and validation matrix without overstating local parity confidence. Added docs CI checks so the public release validation links stay discoverable. Verified with `bash scripts/parity-docs-ci.sh`, `bash scripts/parity-ci-local.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: this is documentation discoverability only; final release confidence still depends on the GitHub Actions matrix and artifact review.
...
- 2026-05-20: Included `assets/icon.svg` in the GitHub release tar/zip packaging steps and added release workflow audit coverage so packaged artifacts keep the brand icon available on all release targets. Verified with `bash scripts/parity-release-workflow-audit.sh`, `cargo check -q`, and `git diff --check`. Remaining risk: this ships the icon in release archives; OS-native app metadata such as `.desktop`, `.icns`, or installer resources remains future packaging work.
