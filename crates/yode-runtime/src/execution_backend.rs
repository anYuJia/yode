...
impl ExecutionBackend for LocalExecutionBackend {
    ...
    async fn availability(&self) -> BackendAvailability {
        BackendAvailability {
            kind: self.kind(),
            available: true,
            detail: "local process execution available".to_string(),
        }
    }
...
impl ExecutionBackend for DockerExecutionBackend {
    ...
    async fn execute(&self, request: &ExecutionRequest) -> Result<ExecutionResult> {
        ...
        if !matches!(self.network.as_str(), "none" | "bridge" | "host") {
            bail!(
                "unsupported Docker network policy '{}'; use none, bridge, or host",
                self.network
            );
        }
        ...
        Ok(to_result(
            ...
            output,
            start,
            json!({"image":self.image,"network":self.network,"workspace":workspace}),
        ))
    }
}
...
impl ExecutionBackend for SshExecutionBackend {
    ...
    async fn execute(&self, request: &ExecutionRequest) -> Result<ExecutionResult> {
        ...
        let remote = format!(
            "cd {} && {}",
            shell_quote(&self.remote_workspace),
            request.command
        );
        ...
        Ok(to_result(
            ...
            output,
            start,
            json!({"target":self.target,"remote_workspace":self.remote_workspace}),
        ))
    }
}
...
impl ExecutionBackend for CloudExecutionBackend {
    ...
    async fn availability(&self) -> BackendAvailability {
        ...
        match client {
            Ok(client) => match client.get(&url).send().await {
                Ok(response) if response.status().is_success() => BackendAvailability {
                    kind: self.kind(),
                    available: true,
                    detail: format!("cloud endpoint reachable: {}", self.endpoint),
                },
                Ok(response) => BackendAvailability {
                    kind: self.kind(),
                    available: false,
                    detail: format!("cloud health returned {}", response.status()),
                },
    ...
    async fn execute(&self, request: &ExecutionRequest) -> Result<ExecutionResult> {
        ...
        let mut result: ExecutionResult = response
            .json()
            .await
            .context("invalid cloud execution response")?;
...
async fn command_availability(
    kind: ExecutionBackendKind,
    executable: &str,
    args: &[&str],
) -> BackendAvailability {
    match Command::new(executable).args(args).output().await {
        Ok(output) if output.status.success() => BackendAvailability {
            kind,
            available: true,
            detail: format!("{executable} available"),
        },
        Ok(output) => BackendAvailability {
            kind,
            available: false,
...
fn ensure_workspace(path: &Path) -> Result<()> {
    if !path.is_dir() {
        bail!("execution workspace is not a directory: {}", path.display());
    }
    Ok(())
}
...
fn validate_remote_path(path: &str) -> Result<()> {
    if path.trim().is_empty() || path.contains('\n') || path.contains('\r') || path.contains('\0') {
        bail!("invalid remote workspace path");
    }
    Ok(())
}
...
fn validate_env_key(key: &str) -> Result<()> {
    ...
    {
        bail!("invalid environment variable key '{key}'");
    }
    Ok(())
}
