...
impl MultiRepoPlan {
    pub fn validate(&self) -> Result<()> {
        if self.repositories.is_empty() {
            bail!("multi-repo plan requires at least one repository");
        }
        if self.steps.is_empty() {
            bail!("multi-repo plan requires at least one step");
        }
        ...
        for repo in &self.repositories {
            ...
            if canonical_paths.iter().any(|existing: &PathBuf| {
                existing == &canonical
                    || canonical.starts_with(existing)
                    || existing.starts_with(&canonical)
            }) {
                bail!(
                    "multi-repo targets must be distinct, non-nested repository roots: {}",
                    canonical.display()
                );
            }
        ...
        for step in &self.steps {
            ...
            if !repo_ids.contains(&step.repo_id) {
                bail!(
                    "step '{}' references unknown repository '{}'",
                    step.id,
                    step.repo_id
                );
            }
        ...
        if scheduled != self.steps.len() {
            bail!("multi-repo plan contains a dependency cycle");
        }
        Ok(())
    }
    ...
    for batch in &batches {
        if batch.iter().any(|step_id| {
            steps.get(step_id).is_some_and(|step| {
                step.depends_on
                    .iter()
                    .any(|dep| results.get(dep).is_some_and(|result| !result.success))
            })
        }) {
                    ...
                    .any(|dep| results.get(dep).is_some_and(|result| !result.success))
                {
                    results.insert(
                        ...
                        MultiRepoStepResult {
                            ...
                            success: false,
                            summary: "Skipped because a dependency failed.".to_string(),
                            artifact_path: None,
                            merged_commit: None,
                        },
                    );
                        ...
                        match finalize.status {
                            ...
                            _ => {
                                ...
                                result.summary = format!(
                                    "{}\nWorktree merge blocked: {}",
                                    result.summary, finalize.message
                                );
                            }
                        }
                    } else {
                        result.summary.push_str(&format!(
                            "\nFailed isolated work retained at {} on branch {}.",
                            lease.path.display(),
                            lease.branch
                        ));
                    }
...
mod tests {
    ...
    fn plans_cross_repo_dependencies_in_parallel_batches() {
        ...
        let plan = MultiRepoPlan {
            ...
            steps: vec![
                ...
                MultiRepoStep {
                    id: "integration".into(),
                    repo_id: "ui".into(),
                    description: "verify integration".into(),
                    depends_on: vec!["api-change".into(), "ui-change".into()],
                    mutating: false,
                },
            ],
        };
        ...
        assert_eq!(
            plan.parallel_batches().unwrap(),
            vec![
                vec!["api-change".to_string(), "ui-change".to_string()],
