...
pub(super) fn read_transcript_metadata(path: &Path) -> Option<TranscriptMetadata> {
...
pub(super) fn extract_summary_preview(content: &str) -> Option<String> {
...
pub(super) fn transcript_picker_summary_preview(path: &Path) -> Option<String> {
    let content = fs::read_to_string(path).ok()?;
