...
impl AgentEngine {
    ...
    pub(super) async fn run_post_tool_use_hooks(
        &self,
        tool_call: &ToolCall,
    ...
    pub(super) async fn execute_advisory_hooks(&mut self, event: HookEvent, context: HookContext) {
        let Some(hook_mgr) = self.hook_manager.as_ref() else {
            return;
        };
    ...
    pub(super) async fn append_hook_outputs_as_system_message(
        &mut self,
        event: HookEvent,
        context: HookContext,
        banner: &str,
    ...
    pub(super) fn build_compaction_hook_context(
        &self,
        event: HookEvent,
        mode: &'static str,
        prompt_tokens: u32,
    ...
    pub(super) fn runtime_hook_metadata(&self) -> Map<String, Value> {
        let mut metadata = Map::new();
        metadata.insert(
